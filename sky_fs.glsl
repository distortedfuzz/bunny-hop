...
vec3 kd = vec3(0.7, 0.6, 0.2);     // diffuse reflectance coefficient
vec3 ka = vec3(0.3, 0.3, 0.3);   // ambient reflectance coefficient
vec3 ks = vec3(1.0, 1.0, 1.0);   // specular reflectance coefficient
vec3 lightPos = vec3(3, 5, 10);   // light position in world coordinates
...
void main(void)
{
    ...
    vec4 tex_color = texture(o_texture, v_texCoord);
    fragColor = tex_color;
